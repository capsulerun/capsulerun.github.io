...
date: "2026-04-28"
description: "Bash is designed for humans and it should stay that way. When i talk about untrusted contexts, I mean automate processes and AI agents. Both scenarios come to the same conclusion: they trying to act like humans, but in reality, don't have the same constraints."
---

Bash is designed for humans and it should stay that way. When i talk about untrusted contexts, I mean automate processes and AI agents. Both scenarios come to the same conclusion: they trying to act like humans, but in reality, don't have the same constraints.
...
Bash delivers this experience perfectly for humans, but not for untrusted processes.
...
There are three types of commands: Observation (ls, grep, cat), Mutation (rm, mv, mkdir), and State (cd, export), which alter the current context.
...
In fact, mutation commands can be pre-created and handwritten, so we might think sandboxing is less important. But for cases like python3, where an agent executes arbitrary code, it becomes essential. That's why we absolutely want to natively sandbox every logic execution.

<SandboxAnimation />

So, it's interesting to keep it natively sandboxed and additionally provide its own workspace to not pollute the user's file system.
